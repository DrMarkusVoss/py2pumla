...
def identifyMe():
    ...
    print("pytopumla v0.2 - by Dr. Markus Voss")
...
def createClassPUMLCode(classelement, targetpath):
    ...
    # write file here
    tfn = targetpath + "/C" + classelement.__name__ + ".puml"
...
def createModulePUMLCore(module, filename, list_of_elementalias, mymod_func, mymod_uses, targetpath):
    ...
    if not(mymod_func == ""):
        ltw = ltw + '\tclass "Module Functions" <<Python>> {\n'
        ltw = ltw + mymod_func
        ltw = ltw + '\t}\n\n'
    ...
    tfn = targetpath + "/" + os.path.basename(filename).split(".")[0] + ".puml"
    with open(tfn, "w") as fil:
        fil.write(ltw)
